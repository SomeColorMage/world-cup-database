...
echo "$($PSQL "SELECT DISTINCT(name) FROM teams LEFT JOIN games AS win_game ON teams.team_id = win_game.winner_id LEFT JOIN games AS lose_game ON teams.team_id = lose_game.opponent_id WHERE win_game.year = 2014 OR lose_game.year = 2014")"
...
echo "$($PSQL "SELECT name FROM teams WHERE name LIKE 'Co%'git ")"
